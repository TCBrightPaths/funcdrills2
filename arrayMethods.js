...
let reduce = (acc, curr) => acc + curr; 
let totalPopulation = populations.reduce(reduce);
...
const myStrongest = monstersInYourPocket.filter(filterCP);
...
function taxRate (item) {
  let newPrice = item.price * (1 + item.tax);
  return Number(newPrice.toFixed(2));
}

let newArray = orders.map(taxRate);
